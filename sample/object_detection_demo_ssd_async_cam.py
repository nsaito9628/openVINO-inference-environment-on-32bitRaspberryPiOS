...
import sys
sys.path.append('/opt/intel/openvino/python/python3.7/openvino/inference_engine')
import os
from argparse import ArgumentParser
import cv2
import time
import logging as log
from ie_api import IENetwork, IEPlugin
...
def main():
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        #initial_w = cap.set(3,1280)
        #initial_h = cap.set(4,720)
        initial_w = cap.get(3)
        initial_h = cap.get(4)
        # Main sync point:
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            for obj in res[0][0]:
                ...
                if obj[2] > args.prob_threshold:
                    ...
                    cv2.circle(frame, (int(xmax-(xmax-xmin)/2), int(ymax-(ymax-ymin)/2)), 30, (0, 0, 255), -1)
